...
def create_html_from_memory_state(memory_state, session):
    """
    ...
    for memory_part in memory_state:
        ...
        if hole != " hole" and domain_shared != " domain-shared":
            ...
            mapping_device = (
                ...
                .filter(Device.id == memory_part["devices_id"])
                .one_or_none()
            )  # not a clean way of doing it !
            dev_name = mapping_device.name
        elif hole != " hole":
            dev_name = "Domain shared space"
            color_id = ""
        elif domain_shared != " domain-share":
            dev_name = "Hole"
            color_id = ""
...
def write_html_file(type=None, value=None):
    ...
    print("Everything worked fine, go check your generated HTML in your browser !")
