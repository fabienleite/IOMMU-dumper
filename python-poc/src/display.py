...
for addr in addresses:
    cnt += 1
    biggest_len = len(addr["iova"]) if len(addr["iova"]) > len(addr["physical_address"]) else len(addr["physical_address"])
    biggest_len_is_virt = True if len(addr["iova"]) > len(addr["physical_address"]) else False
